 ...
 */
package uk.gov.hmrc.components.test.molecule.status
...
object StatusViewMatchers {

    fun hasTitle(text: String): Matcher<View> {
        return object : BoundedMatcher<View, StatusView>(StatusView::class.java) {
            override fun describeTo(description: Description?) {
                description?.appendText("has title = $text")
            }

            override fun matchesSafely(item: StatusView?): Boolean {
