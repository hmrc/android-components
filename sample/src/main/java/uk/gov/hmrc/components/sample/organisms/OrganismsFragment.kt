...
class OrganismsFragment : ComponentListFragment() {
    ...
    override fun provideItems(): List<ComponentListItem> {
        return listOf(
            ...
            ComponentListItem(getString(R.string.organisms_editable_list_view)) { selectFragment(EditableListViewFragment()) },
            ComponentListItem(getString(R.string.organisms_mini_advert_card)) { selectFragment(MiniAdvertCardFragment()) }
        )
    }
}
